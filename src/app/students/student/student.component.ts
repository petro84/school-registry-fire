...
export class StudentComponent implements OnInit {
      ...
      this.studentsSvc.createStudent(this.teacherId, student).subscribe({
        next: () =>
          this.createToastMsg(
            'success',
            'Success!',
            `${student.firstName} ${student.lastName} has been assigned to ${teacher.title} ${teacher.lastName}'s class.`,
            false
          ),
        error: (err) => {
          this.createToastMsg(
            'error',
            'Error',
            `Unable to register ${student.firstName} ${student.lastName} at this time. Please try again later.`,
            true
          );
          console.error(err);
        },
      ...
      deletePreviousStudent.subscribe({
        next: () =>
          this.createToastMsg(
            'success',
            'Success!',
            `${student.firstName} ${student.lastName} has been transfered to ${teacher.title} ${teacher.lastName}'s class.`,
            false
          ),
        error: (err) => {
          this.createToastMsg(
            'error',
            'Error',
            `System error, unable to modify ${student.firstName} ${student.lastName} at this time. Please try again later.`,
            true
          );
          console.error(err);
        },
      ...
      this.studentsSvc.updateStudent(this.teacherId, sId, student).subscribe({
        next: () =>
          this.createToastMsg(
            'success',
            'Success!',
            `${student.firstName} ${student.lastName} has been updated.`,
            false
          ),
        error: (err) => {
          this.createToastMsg(
  ...
  delete() {
    if (this.teacherId && this.student?.studentId) {
      this.studentsSvc
        .deleteStudent(this.teacherId, this.student.studentId)
        .subscribe({
          next: () =>
            this.createToastMsg(
              'success',
              'Success!',
              `${this.student?.firstName} ${this.student?.lastName} has been removed.`,
              false
            ),
          error: (err) => {
            this.createToastMsg(
              'error',
              'Error',
              `System error, unable to remove ${this.student?.firstName} ${this.student?.lastName} at this time. Please try again later.`,
              true
            );
            console.error(err);
          },
